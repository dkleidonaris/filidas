	...
	>
		<div class="flex flex-col h-screen backdrop-blur-[2px]">
            <div class="mt-64 md:my-60 ml-8 md:ml-40 flex flex-col gap-4 tracking-[5px]">
                <h2 x-data="{show: false}" x-show="show" x-init="setTimeout(() => {show= true}, 500);" x-transition class="text-2xl md:text-4xl text-yellow-400" x-cloak>{{Str::gr_strtoupper(__('Διαμερίσματα'))}}</h2>
                <h1 x-data="{show: false}" x-show="show" x-init="setTimeout(() => {show= true}, 700);" x-transition class="text-3xl md:text-7xl font-bold drop-shadow-lg text-white" x-cloak>{{__('Διαμερίσματα FILIDAS')}}</h1>

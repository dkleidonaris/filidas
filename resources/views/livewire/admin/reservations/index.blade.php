...
new class extends Component {
    ...
    {
        $this->filtered_reservations = Reservation::when(!empty($this->customer_q), function ($q) {
    ...
    public function updatedCustomerQ()
    {
        $this->filterReservations();
    }
}; ?>
...
<div class="space-y-4 p-4">
    ...
    <div>
        <input
            type="text"
            wire:model.live.debounce.300ms="customer_q"
            ...
            <tbody>
                @forelse($filtered_reservations as $reservation)
                    ...
                    >
                        ...
                        <td class="px-4 py-2">
                            {{ $reservation->customer->email }}</td>
                        <td class="px-4 py-2">
                            {{ $reservation->created_at->format('d/m/Y') }}
                        </td>
                        <td class="px-4 py-2">
                            {{ optional($reservation->checkin_date)->format('d/m/Y') }}
                        </td>
                        <td class="px-4 py-2">
                            {{ optional($reservation->checkout_date)->format('d/m/Y') }}
                        </td>
